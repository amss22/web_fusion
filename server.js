...
function initDb() {
  db.serialize(() => {
    ...
    // Check if initial users exist, else seed
    db.get('SELECT COUNT(*) as count FROM users', (err, row) => {
      if (row && row.count === 0) {
        ...
        db.run(`INSERT INTO platform_config VALUES (1, 5, 10, 25, 200, 24, 1, 0)`);
      }
    });
...
app.post('/api/auth/login', (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE email = ? AND role = ?', [email, role], (err, user) => {
    ...
    if (!user) return res.status(401).json({ error: 'User not found for this role' });
...
app.post('/api/reset-db', (req, res) => {
  db.serialize(() => {
    ...
    initDb();
    res.json({ message: 'Local SQLite database wiped and reset to clean initial state.' });
  });
});
